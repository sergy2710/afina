...
private:
    ...
    /**
     * List of blocked coroutines (on IO)
     */
    context *blocked;
...
protected:
     ...
     */
    void MoveCoroutine(context *&fromlist, context *&tolist, context *routine);

public:
    Engine()
        : StackBottom(0), idle_func([]() {}), cur_routine(nullptr), alive(nullptr), blocked(nullptr),
          idle_ctx(nullptr) {}
    ...
    /**
     * Check if all the coroutines are blocked
     */
    bool all_blocked() const;
    ...
    void start_noargs(std::function<void()> main) {
        // To acquire stack begin, create variable on stack and remember its address
        char StackStartsHere;
        this->StackBottom = &StackStartsHere;

        // Start routine execution
        void *pc = run_noargs(main);
        idle_ctx = new context();
    ...
    void *run_noargs(std::function<void()> func) {
        if (this->StackBottom == 0) {
            // Engine wasn't initialized yet
            return nullptr;
        }
