...
bool SimpleLRU::Delete(const std::string &key) {
    ...
    if (elem_it == _lru_index.end())
        ...
        if (!SimpleLRU::RecountCurrentSize((-1) * (found_node->key.size() + found_node->value.size())))
            return false;
...
bool SimpleLRU::ClearMemory(const std::size_t needed_size) {
    if (needed_size <= _current_size) {
        ...
        if (!SimpleLRU::RecountCurrentSize((-1) * cleared_size))
            return false;
...
bool SimpleLRU::RecountCurrentSize(const std::size_t increment) {
    ...
    if ((new_size < 0) | (new_size > _max_size))
        return false;
    else {
    ...
    if (increment <= _max_size) // to not destroy elements if we don't have enough memory after freeing
    {
        if (_empty_size < increment)
            if (!SimpleLRU::ClearMemory(increment))
                return false;
        if (!SimpleLRU::RecountCurrentSize(increment))
            return false;
